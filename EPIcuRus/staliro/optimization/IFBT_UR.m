function [run, history] = IFBT_UR(inpRanges,opt,Oldt,controlPointNames,categorical,count)
% IFBT_UR(UR based Important Features Boundary Test) - generates test cases by focusing mainly on the important features that have the highest impact on the fitness
%           values given the previously generated test cases and the previously generated assumptions
% USAGE:
%   [run, history] = IFBT_UR(inpRanges,opt,Oldt,controlPointNames,categorical,count)
%
% INPUTS:
%
        ...
        for rangetc = start+1 : endd+1
            ...
            if opt.dispinfo>0
                if (mod(tc,floor(100/opt.n_workers)) == 0)
                    disp([' IFBT_UR_Taliro: Number of tests so far ',num2str(tc*opt.n_workers)])
                end
            end
