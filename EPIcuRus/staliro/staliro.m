...
for ii=1:staliro_opt.runs
    ...
    RUNSTATS.new_run();
    if nargout>1
        if (strcmp(staliro_opt.optimization_solver,'IFBT_UR_M')) &&  isempty(Oldt)
%             [getRun, getHistory] = feval('UR_Taliro',InpRange,staliro_opt,Oldt,controlPointNames, categorical,count);
            [getRun, getHistory] = feval('UR_Taliro_M',model,InpRange,staliro_opt,Oldt,controlPointNames, categorical,count);
        else
            if (strcmp(staliro_opt.optimization_solver,'IFBT_ART_M')) &&  isempty(Oldt)
                [getRun, getHistory] = feval('AR_Taliro',InpRange,staliro_opt,Oldt,controlPointNames,categorical,count);
            else
                [getRun, getHistory] = feval(staliro_opt.optimization_solver,model,InpRange,staliro_opt,Oldt,controlPointNames,categorical,count);
%                 [getRun, getHistory] = IFBT_UR_M(InpRange,staliro_opt,Oldt,controlPointNames,categorical,count);
            end
        end
        history(ii) = getHistory;
    else
        if (strcmp(staliro_opt.optimization_solver,'IFBT_UR')||strcmp(staliro_opt.optimization_solver,'IFBT_ART')) &&  isempty(Oldt)
            getRun = feval('UR_Taliro',InpRange,staliro_opt,Oldt,controlPointNames,count);
        else
            getRun = feval(staliro_opt.optimization_solver,InpRange,staliro_opt,Oldt,controlPointNames,categorical,count);
        end
    end
