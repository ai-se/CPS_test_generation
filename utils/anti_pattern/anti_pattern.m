% function anti_pattern - Computing the anti-pattern value
%
% USAGE
%
%         rob = anti_patter(seqS,seqT,seqL,customCostAuxData)
%
% INPUTS
%
...
function varargout = anti_pattern(seqS, seqT)
seqS = double(seqS);
deltaT = seqT(2) - seqT(1);

if isempty(seqS) || isempty(seqT)
    error('anti_pattern: The output signal or time stamp is empty.')
else  
    [r, c] = size(seqS);
    score = zeros(1, c*4);
...
if nargout == 1
    ...
    error(' anti_pattern: The maximum number of outputs is one.')
end
